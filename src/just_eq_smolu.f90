...
subroutine just_eq_smolu
  ...
  use system, only: D_plus, D_minus, &
                     D_equil, time, inside, lx, ly, lz, c, fluid, phi, delta, c_plus, c_minus,&
                     rho_0=>rho_ch, sigma, NbVel, plusx, plusy, plusz
  use constants, only: x, y, z
  implicit none

  integer(kind=i2b) :: iter, max_iter
  real(kind=dp), dimension(lx,ly,lz) :: flux_site_plus, flux_site_minus
  integer(kind=i2b) :: i, j, k, ip, jp, kp, l ! dummy
  real(kind=dp) :: exp_dphi, exp_min_dphi ! Exp[phi site1 - phi site 2] and 1/Exp
  real(kind=dp) :: flux_link_plus, flux_link_minus
  real(kind=dp) :: tot_diff_plus, tot_diff_minus ! total flux of + and - solutes init at high values
  real(kind=dp) :: eD_plus, eD_minus ! effective D_plus and D_minus
  real(kind=dp), parameter :: convergence_criteria = 5.e-6
  ...
  do while( tot_diff_minus + tot_diff_plus > convergence_criteria .and. iter < max_iter )
    ...
    do i= 1, lx
      do j= 1, ly
        do k= 1, lz
          ...
          ! here one could add a if(inside(i,j,k)==solid)cycle ! but things get hard to read
          ...
          do l= 2, NbVel, 2 ! at once flux in both directions ! l=1 corresponds to no velocity ie flux toward itself ie delta(l)=0
            ...
            if( inside(i,j,k) == fluid .and. inside(ip,jp,kp) == fluid) then
              ...
              flux_link_plus = flux_link_plus * eD_plus / delta(l)
              flux_link_minus = 0.5_dp * (1.0_dp+ exp_dphi    )*( c_minus(ip,jp,kp) * exp_min_dphi - c_minus(i,j,k) )
              flux_link_minus = flux_link_minus * eD_minus / delta(l)
    ...
    if( sigma/=0 .and. eD_plus/=0.0_dp .and. eD_minus/=0.0_dp ) then
      ! the sum of all flux
      tot_diff_plus  = sqrt(sum(flux_site_plus**2,mask=(inside==fluid)))  / count(inside==fluid) / (0.5_dp*rho_0*eD_plus ) / sigma ! 1st denominator is the number of fluid nodes)
      tot_diff_minus = sqrt(sum(flux_site_minus**2,mask=(inside==fluid))) / count(inside==fluid) / (0.5_dp*rho_0*eD_minus) / sigma ! norm2 is the Fortran intrinsic for euclidean norm
