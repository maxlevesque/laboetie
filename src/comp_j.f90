...
subroutine comp_j
  ...
  use system, only: jx, jy, jz, c, n, lx, ly, lz !, f_ext, inside, solid
...
!        jx(i,j,k) = sum( n(i,j,k,:)*c(x,:) )
!        jy(i,j,k) = sum( n(i,j,k,:)*c(y,:) ) 
!        jz(i,j,k) = sum( n(i,j,k,:)*c(z,:) ) 
...
! Much better than the naive conventional definition in terms of numerical stability.
        jx(i,j,k) = 0.5_dp*(jx(i,j,k) + sum( n(i,j,k,:)*c(x,:) ))
        jy(i,j,k) = 0.5_dp*(jy(i,j,k) + sum( n(i,j,k,:)*c(y,:) ))
        jz(i,j,k) = 0.5_dp*(jz(i,j,k) + sum( n(i,j,k,:)*c(z,:) ))
...
!        jx(i,j,k) = sum( n(i,j,k,:)*c(x,:) ) + f_ext(x)/2.0_dp
!        jy(i,j,k) = sum( n(i,j,k,:)*c(y,:) ) + f_ext(y)/2.0_dp
!        jz(i,j,k) = sum( n(i,j,k,:)*c(z,:) ) + f_ext(z)/2.0_dp
